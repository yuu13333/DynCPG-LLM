...
# 下载 GitHub commit 的快照（ZIP 文件）
def download_commit_snapshot(logger, owner, repo, commit_hash, save_dir):
    url = f"https://bgithub.xyz/{owner}/{repo}/archive/{commit_hash}.zip"
    # url = f"https://github.ur1.fun/{owner}/{repo}/archive/{commit_hash}.zip"
    print(url)
    # 构建 curl 命令
    # 确保保存目录存在
